...
def winget_install(package):
    ...
    subprocess.run(["powershell", "-Command", "winget", "install", package, "--accept-package-agreements", "--accept-source-agreements"], shell=True, check=True)
...
def choco_install(package):
    try:
        subprocess.run([
            ...
            "--not-silent", "--failonstderr",
            "--no-silent", "--interactive", "-y"
        ])
...
# Scoop Install Apps
def scoop_install(package_list):
...
# Script to download files that aren't available through package manager(s)
def download_file(url, filename):
    try:
        userprofile = os.environ.get('USERPROFILE')
        downloads_folder = os.path.join(userprofile, "Downloads")
...
# Changes Raphire's text file to custom one.
def raphire_install_txt_change():
    ...
    # Checks and remove folders if already existent
    ...
    # Delete Raphire's .txt
    if os.path.exists(raphiretxt2):
        os.remove(raphiretxt2)

    # Copy Custom .txt to Raphire's Folder
...
# Executes modified script with Custom .txt
...
# Executes RamMap cleaning everything        
def ram_map():
    try:
...
# Creates task and Define schedule for RamMap to 30min
def schedule_rammap():
...
# Removes Windows Defender using ionuttbara's script.  
...
def ms_edgeR():
    ...
    # Checks and remove folder if already exists
...
# Downloads using Microsoft Store as source for Winget
def winget_ms(pkg):
    try:
...
# Installs Git on the machine
def git_setup():
    try:
        git = subprocess.run(["powershell", "-Command", "git"],
                       shell=True, capture_output=True, text=True
                       )
        if git.returncode != 0:
            winget_silent("Git.Git")
        else:
            print("Git is already installed")
    except subprocess.CalledProcessError as e:
        print("An error occured:", e.stderr, e.stdout)            

# Installs Choco on the machine
def choco_setup():
    try:
...
# Installs Scoop on the machine    
def scoop_setup():
    try:
        # Check if Scoop is installed
        scoop = subprocess.run(["powershell", "-Command", "scoop"], shell=True, capture_output=True, text=True)
        if scoop.returncode != 0:
